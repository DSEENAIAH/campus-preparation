...
const AdminDashboard = () => {
  ...
  const [activeTestEndTime, setActiveTestEndTime] = useState(null);
  ...
  // Check if test is currently being taken by any student
  const checkTestActivity = useCallback(async (testId) => {
    try {
      const command = new ScanCommand({
        TableName: AWS_CONFIG.tables.progress || 'codenvia-exam-progress',
        FilterExpression: 'testId = :testId AND attribute_not_exists(completedAt)',
        ExpressionAttributeValues: { ':testId': testId }
      });
      
      const result = await docClient.send(command);
      const activeProgress = result.Items || [];
      
      if (activeProgress.length > 0) {
        // Test is active, find the earliest end time
        const endTimes = activeProgress
          .map(progress => {
            if (progress.startedAt && progress.timeLimit) {
              const startTime = new Date(progress.startedAt);
              const endTime = new Date(startTime.getTime() + (progress.timeLimit * 60 * 1000));
              return endTime;
            }
            return null;
          })
        ...
        if (endTimes.length > 0) {
          const earliestEndTime = new Date(Math.min(...endTimes.map(t => t.getTime())));
          return { isActive: true, endTime: earliestEndTime };
        }
      }
      
      return { isActive: false, endTime: null };
    } catch (error) {
      console.error('Error checking test activity:', error);
      return { isActive: false, endTime: null };
    }
  }, [docClient]);
  ...
  const fetchExamProgress = useCallback(async () => {
    try {
      ...
      console.log('📊 Raw progress items:', result.Items?.length || 0);
      console.log('⏰ Active exams (last 2 min):', activeExams.length);
      console.log('🔄 Deduplicated active exams:', deduplicatedExams.length);
      
      if (deduplicatedExams.length === 0) {
        console.log('✅ No active exams found - clearing display');
      }
      ...
      cleanupIntervalId = setInterval(async () => {
        try {
          ...
          if (result.Items && result.Items.length > 0) {
            console.log(`🧹 Cleaning up ${result.Items.length} stale progress records`);
  ...
  const handleReassignTest = useCallback(async (test) => {
    ...
    try {
      ...
      const activityStatus = await checkTestActivity(test.id);
      setIsTestActive(activityStatus.isActive);
      setActiveTestEndTime(activityStatus.endTime);
      ...
      setReassignData({
        ...
        expiryDate: activityStatus.isActive ? activityStatus.endTime?.toISOString().slice(0, 16) : '',
        expiryTime: activityStatus.isActive ? activityStatus.endTime?.toISOString().slice(0, 16) : '',
        useActiveTime: activityStatus.isActive
      });
  ...
  const confirmReassignTest = useCallback(async () => {
    ...
    // Validate expiry time when test is not active
    if (!isTestActive && !reassignData.expiryTime) {
      showNotificationMessage('Please set an expiry date and time for the test assignment', 'warning');
      return;
    }
    ...
    try {
      ...
      const assignmentPromises = reassignData.toStudents.map(async (studentEmail) => {
        ...
        // Determine due date based on test activity status
        let dueDate = null;
        if (isTestActive && activeTestEndTime) {
          // Use active test end time
          dueDate = activeTestEndTime.toISOString();
        } else if (reassignData.expiryTime) {
          // Use manually set expiry time
          dueDate = new Date(reassignData.expiryTime).toISOString();
        }
        ...
        const assignment = {
          ...
          currentAttempts: 0,
          expiresAt: dueDate,
          assignmentType: isTestActive ? 'active_reassign' : 'scheduled_reassign'
        };
    ...
    <div className="admin-dashboard-container">
      ...
      <nav className="admin-nav">
        <div className="nav-tabs">
          ...
          <button 
            className={`nav-tab ${activeTab === 'manage-tasks' ? 'active' : ''}`}
        ...
        <Suspense fallback={
          <div className="loading-container">
            <div className="loading-spinner">
              ...
              <span>Loading Manage Tasks...</span>
            </div>
          </div>
        }>
          <ManageTasksTab
              ...
              <div className="test-preview">
                ...
                {(selectedTest.realTimeStats?.totalQuestions > 0 || (selectedTest.questions && selectedTest.questions.length > 0)) && (
                  <div className="preview-questions">
                    <h5>📝 Question Breakdown by Module:</h5>
      ...
      {showScheduleModal && selectedTest && (
        <div className="modal-overlay" onClick={() => setShowScheduleModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>📅 Schedule Test {selectedTest.college ? `for ${selectedTest.college}` : ''}</h3>
                  ...
                  <div className="time-settings-section">
                    <div className="section-header">
                      ...
                      <span className="section-description">
                        {isTestActive 
                          ? "Test is currently active - new students will use the same end time" 
                          : "Set expiry time for newly assigned students"
                        }
                      </span>
                    </div>
                      ...
                      <div className="active-test-info">
                        <div className="info-card">
                          ...
                          <div className="info-content">
                            <div className="info-title">Test Currently Active</div>
                            <div className="info-details">
                              <span>Students are taking this test right now</span>
                              <span>New assignments will expire at: <strong>{activeTestEndTime?.toLocaleString()}</strong></span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="expiry-settings">
                        <div className="form-group">
                          ...
                          <span className="field-hint">
                            Set when the test assignment should expire for new students
                          </span>
                        </div>
                      </div>
                    )}
            ...
            <div className="modal-footer">
              ...
              <button className="btn-info" onClick={() => {
                setReassignData({
                  ...
                  notifyStudents: true,
                  reassignType: 'individual',
                  expiryDate: isTestActive && activeTestEndTime ? activeTestEndTime.toISOString().slice(0, 16) : '',
                  expiryTime: isTestActive && activeTestEndTime ? activeTestEndTime.toISOString().slice(0, 16) : '',
                  useActiveTime: isTestActive
                });
              }}>
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{marginRight: '0.25rem'}}>
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            ...
            <div className="modal-body">
              <div className="analytics-dashboard">
                <div className="analytics-stats">
                  <div className="stat-card">📈 Completion Rate: 85%</div>
                  <div className="stat-card">⭐ Average Score: 78%</div>
                  <div className="stat-card">⏱️ Avg Time: 45 min</div>
                  <div className="stat-card">👥 Total Attempts: 156</div>
                </div>
                <div className="analytics-charts">
                  <div className="chart-placeholder">📊 Score Distribution Chart</div>
                  <div className="chart-placeholder">📈 Performance Trends</div>
                </div>
              </div>
            </div>
            <div className="modal-footer">
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="bulk-options">
                <button className="bulk-option">📢 Publish All</button>
                <button className="bulk-option">📦 Archive All</button>
                <button className="bulk-option">📅 Schedule All</button>
                <button className="bulk-option danger">🗑️ Delete All</button>
              </div>
            </div>
            <div className="modal-footer">
        ...
        <div className="modal-overlay" onClick={() => setShowPublishModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="close-btn" onClick={() => setShowPublishModal(false)}>×</button>
            </div>
            <div className="modal-body">
              <div className="publish-form">
                <div className="form-group">
                  <label>Select Colleges to Publish</label>
                  <p className="form-help">Leave empty to publish globally to all students</p>
                ...
                <div className="publish-summary">
                  <p><strong>Selected Colleges:</strong> {publishData.selectedColleges.length === 0 ? 'All Colleges (Global)' : publishData.selectedColleges.length}</p>
                  <p><strong>Total Students:</strong> {publishData.selectedColleges.length === 0 ? students.length : publishData.selectedColleges.reduce((sum, college) => sum + students.filter(s => s.college === college).length, 0)}</p>
                </div>
              </div>
            </div>
            <div className="modal-footer">
              ...
              <button
                className="btn-primary"
                disabled={publishData.publishType === 'schedule' && !publishData.startDate}
                onClick={async () => {
                  try {
                    // Update test status
                    await docClient.send(new UpdateCommand({
                      TableName: AWS_CONFIG.tables.tests,
                      Key: { id: selectedTest.id },
                      UpdateExpression: 'SET #status = :status, updatedAt = :updatedAt, publishedAt = :publishedAt',
                      ExpressionAttributeNames: { '#status': 'status' },
                      ExpressionAttributeValues: {
                        ':status': 'published',
                        ':updatedAt': new Date().toISOString(),
                    ...
                    // Create assignments for selected colleges or all students if none selected
                    const assignmentPromises = [];
                    const timestamp = new Date().toISOString();
                    
                    if (publishData.selectedColleges.length === 0) {
                      // Global publish - assign to all students
                      students.forEach(student => {
                        const assignmentId = `assignment_${selectedTest.id}_${student.email}_${timestamp.replace(/[:.]/g, '')}`;
                    ...
                    const totalStudents = publishData.selectedColleges.length === 0 
                      ? students.length 
                      : publishData.selectedColleges.reduce((sum, college) => sum + students.filter(s => s.college === college).length, 0);
                    
                    const message = publishData.selectedColleges.length === 0
                      ? `Test published globally to all ${totalStudents} students! Students can now see this test in their dashboard.`
                      : `Test published to ${publishData.selectedColleges.length} colleges (${totalStudents} students)! Students can now see this test in their dashboard.`;
                    
                    showNotificationMessage(message, 'success');
                    setShowPublishModal(false);
                  } catch (error) {
                    console.error('Error publishing test:', error);
                    showNotificationMessage('Failed to publish test. Please try again.', 'error');
                  }
                }}
              >
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{marginRight: '0.25rem'}}>
                ...
                {publishData.publishType === 'now' ? 'Publish Now' : 'Schedule'} {publishData.selectedColleges.length === 0 ? 'Globally' : `to ${publishData.selectedColleges.length} Colleges`}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setShowAssignTestsModal(false)}>
          <div className="modal-content large" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="close-btn" onClick={() => setShowAssignTestsModal(false)}>×</button>
            </div>
            <div className="modal-body">
        ...
        <div className="modal-overlay" onClick={() => setShowAssignmentsModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="close-btn" onClick={() => setShowAssignmentsModal(false)}>×</button>
            </div>
            <div className="modal-body">
                      ...
                      <div className="assignment-header">
                        <div className="college-info">
                          ...
                          <div className="assignment-meta">
                            <span className="student-count">👥 {assignment.studentCount} students</span>
                            <span className="assigned-date">📅 {new Date(assignment.assignedAt).toLocaleDateString()}</span>
                          </div>
                        </div>
